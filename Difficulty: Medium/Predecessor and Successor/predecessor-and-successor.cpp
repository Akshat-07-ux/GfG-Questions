...
public:
    ...
    void findPreSuc(Node* root, Node*& pre, Node*& suc, int key) {
        ...
        // If the root's key is equal to the given key
        if (root->key == key) {
        ...
        // If key is smaller than the root's key, move to the left subtree
        else if (key < root->key) {
            suc = root;  // possible successor
            findPreSuc(root->left, pre, suc, key);
        }
        // If key is greater than the root's key, move to the right subtree
        else {
