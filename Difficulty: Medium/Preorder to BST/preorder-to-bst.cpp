...
public:
    ...
    Node* Bst(int pre[], int size) {
        ...
        for (int i = 1; i < size; i++) {
            ...
            // Pop ancestors from the stack while the current value is greater than the top of the stack
            while (!s.empty() && pre[i] > s.top()->data) {
                temp = s.top();
                s.pop();
            }

            // If the current value is greater than the last popped node, it is the right child
            if (temp != nullptr) {
                temp->right = newNode(pre[i]);
                s.push(temp->right);
            }
            // Else, it is the left child of the top node
            else {
